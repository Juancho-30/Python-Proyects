...
def view_command():
   ...
   for row in backend.view():
      list1.insert(END, row) #END as index for row to put the new rows at the end always
...
def get_selected_row(event):
   try:
      ...
      index=list1.curselection()[0] #curseselection method from listbox
      print(index)
...
list1.bind('<<ListboxSelect>>', get_selected_row) #Takes event type, and function as arguments, executed to define a function binding with the object
...
b2=Button(window,text="Search Entry", width=12, command=seach_command)
b2.grid(row=3,column=3)

b3=Button(window,text="Add Entry", width=12, command=command_entry)
